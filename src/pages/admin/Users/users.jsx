...
export default function AdminUsers() {
  ...
  const [page, setPage] = useState(1);                      //current page default 1
  const [totalPages, setTotalPages] = useState(0);          //total pages gana. data tiynwa vidya anuwa 
  ...
  const fetchUsers = () => {
    axios
      ...
      {
        page : page,
        pageSize : 5                          //req eke body eka dala yawanwa.
      },       
      {
        headers: {
      ...
      .then((res) => {
        ...
        setTotalPages(res.data.pagination.totalPages); //total pages gana gannwa backend eken ena respone eken
      })
      .catch((err) => {
        console.error(err);
  ...
  useEffect(() => {
    fetchUsers();
  }, [page]);  //current page eka wenas unoth useEffect run wenwa(fechUsers)
    ...
    <div className="w-full p-6 bg-gray-50">
      <div className="overflow-x-auto">
        ...
        <div className="w-full flex justify-center items-center">
          {
            Array.from({length : totalPages}).map((item,index)=>{    //empty array ekak gannwa totalpages ganata render wenna      (index=0) so that why we use index+1
              return(                                                   //button eka click karama current page eka change wenwa

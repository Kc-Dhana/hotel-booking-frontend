        ...
        <>
            ...
            <div className="p-6 max-w-7xl mx-auto">
                ...
                <div className="bg-white shadow p-4 rounded-lg mb-6 flex flex-wrap gap-4 items-center">
                    <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="border p-2 rounded w-[30%]" />
                    <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="border p-2 rounded w-[30%]" />
                    <select value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)} className="border p-2 rounded w-[30%]">
                    ...
                    <div className="w-3/4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {availableRooms.length > 0 ? (
                            availableRooms.map(room => (
                                <div key={room._id} className="bg-white p-4 rounded shadow relative">
                                    <img src={room.photos?.[0] || "/no-image.jpg"} alt={room.name} className="h-40 w-full object-cover rounded" />
                                    ...
                                    <div className="mt-2 flex gap-2">
                                        <button
                                            ...
                                            className={`w-full py-2 rounded text-white ${isInCart(room) ? "bg-red-500 hover:bg-red-600" : "bg-green-500 hover:bg-green-600"}`}
                                            disabled={!token}
                                        >

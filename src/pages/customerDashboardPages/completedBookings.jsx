...
export default function CompletedBookings() {
  ...
  useEffect(() => {
    ...
    axios
      .get(import.meta.env.VITE_BACKEND_URL + "/api/bookings/customer-bookings?status=completed", {
        headers: {
  ...
  <div className="p-6 sm:p-8 bg-gray-100 min-h-screen">
    <h1 className="text-3xl font-bold text-gray-800 mb-6">Completed Booking Requests</h1>
    ...
    <div className="flex flex-col gap-4 md:hidden mt-4">
      {bookings.map((booking) => (
        <div key={booking.bookingId} className="bg-white p-4 shadow rounded-lg border border-gray-200">
          <p><strong>Booking ID:</strong> {booking.bookingId}</p>
          <p><strong>Room ID:</strong> {booking.roomId}</p>
          <p>
            <strong>Status:</strong>{" "}
            <span className="capitalize text-blue-600 font-semibold">{booking.status}</span>
          </p>
          ...
          <button
            onClick={() => openModal(booking)}
            className="mt-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-1 rounded text-sm w-full"
          >
            View Details
          </button>
        </div>
            ...
            >
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                ...
                <div className="mt-4 space-y-2 text-sm text-gray-700">
                  ...
                  {selectedBooking?.reason && <p><strong>Reason:</strong> {selectedBooking.reason}</p>}
